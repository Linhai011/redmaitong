...
router.get('/platform', optionalAuth, async function (req, res) {
  try {
    ...
    var [uploadRows] = await pool.query(
      'SELECT strftime(\'%Y-%m\', created_at) AS month, COUNT(*) AS cnt FROM resource GROUP BY month ORDER BY month'
    );
    var [shareRows] = await pool.query(
      'SELECT strftime(\'%Y-%m\', created_at) AS month, COUNT(*) AS cnt FROM share_auth GROUP BY month ORDER BY month'
    );
    var shareMap = {};
